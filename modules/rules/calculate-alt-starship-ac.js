...
const SFRPGEffectType = Object.freeze({
...
export function alternityCalculateStarshipArmorClass(fact, context) {
        const data = fact.data;
        const actor = fact.actor;
    const configuredBase = Number(game.settings.get(MODULE_ID, SETTING_KEYS.starshipArmorClassBase));
    const baseArmorClass = Number.isFinite(configuredBase) ? configuredBase : 10;
    const baseTooltip = game?.i18n
        ? game.i18n.format("SFA.StarshipSheet.Modifiers.Base", { value: baseArmorClass })
        : `Base: ${baseArmorClass}`;
...
export default function calculateStarshipArmorClass(engine) {
    engine.closures.add("calculateStarshipArmorClass", alternityCalculateStarshipArmorClass, {
        required: ["stackModifiers"],
        closureParameters: ["stackModifiers"]
    });
}
