import { MODULE_ID, SETTING_KEYS } from "../alternity-constants.js";

export function alternityCalculateStarshipTargetLock(fact) {
        const data = fact.data;
        const actor = fact.actor;
    const configuredBase = Number(game.settings.get(MODULE_ID, SETTING_KEYS.starshipTargetLockBase));
    const baseTargetLock = Number.isFinite(configuredBase) ? configuredBase : 10;
    const baseTooltip = game?.i18n
        ? game.i18n.format("SFA.StarshipSheet.Modifiers.Base", { value: baseTargetLock })
        : `Base: ${baseTargetLock}`;
        ...
        if (pilot?.type === "npc2") {
            pilotingRanks = pilot?.system?.skills?.pil?.ranks - pilot?.system?.abilities?.dex?.mod || 0;
        }
        ...
        /** Get modifying items. */
        /** Don't include defensive countermeasures in the calculation, as they are applied at the time of attack. 
        ...
        console.log("Starship Target Lock Calculation Complete", fact);
/*
...
export default function calculateStarshipTargetLock(engine) {
    engine.closures.add("calculateStarshipTargetLock", alternityCalculateStarshipTargetLock);
}
